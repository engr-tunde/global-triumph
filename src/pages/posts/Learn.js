...
const Learn = () => {
  ...
  return (
    <>
      <TopHead pageTitle="Learn" />
      <section className="learn">
        {data && (
          <Container>
            <div className="learn-bx">
              <h3>Learn</h3>
              <BlogTop learnItem1={data.filter((post) => post.category === 'Learn').slice(0, 1)} />
              <BlogSecondRow
                learnSecondRowItems={data.filter((post) => post.category === 'Learn').slice(1, 4)}
              />
              <BlogMain
                learnMainItems={data.filter((post) => post.category === 'Learn').slice(4, 100)}
              />
            </div>
          </Container>
        )}
